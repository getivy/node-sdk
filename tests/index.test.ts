...
import { APIPromise } from '@getivy/node-sdk/core/api-promise';
...
describe('logging', () => {
  ...
  const forceAPIResponseForClient = async (client: Ivy) => {
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['IVY_LOG'] = 'debug';
    const client = new Ivy({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['IVY_LOG'] = 'not a log level';
    const client = new Ivy({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'IVY_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Ivy({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['IVY_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Ivy({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['IVY_BASE_URL'] = 'https://example.com/from_env';
      const client = new Ivy({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['IVY_BASE_URL'] = ''; // empty
      const client = new Ivy({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.getivy.de')
    });

    test('blank env variable', () => {
      process.env['IVY_BASE_URL'] = '  '; // blank
      const client = new Ivy({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.getivy.de')
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () =>
          new Ivy({ apiKey: 'My API Key', environment: 'production' })
    ...
    test('in request options overridden by client options', () => {
      const client = new Ivy({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['IVY_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Ivy({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Ivy({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Ivy({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['IVY_API_KEY'] = 'My API Key';
    const client = new Ivy();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['IVY_API_KEY'] = 'another My API Key';
    const client = new Ivy({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('idempotency', () => {
  test.skip('key can be set per-request', async () => {
    const client = new Ivy({ baseURL: process.env["TEST_API_BASE_URL"] ?? 'http://127.0.0.1:4010', apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Ivy({ apiKey: 'My API Key', fetch: testFetch });
