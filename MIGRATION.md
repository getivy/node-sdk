...
To use it, upgrade the `@getivy/node-sdk` package, then run `./node_modules/.bin/getivy-node-sdk migrate ./your/src/folders` to update your code.
...
The `httpAgent` client option has been removed in favor of a [platform-specific `fetchOptions` property](https://github.com/stainless-sdks/getivy-typescript#fetch-options).
This change was made as `httpAgent` relied on `node:http` agents which are not supported by any runtime's builtin fetch implementation.

If you were using `httpAgent` for proxy support, check out the [new proxy documentation](https://github.com/stainless-sdks/getivy-typescript#configuring-proxies).
...
#### Refactor of `@getivy/node-sdk/core`, `error`, `pagination`, `resource` and `uploads`

Much of the `@getivy/node-sdk/core` file was intended to be internal-only but it was publicly accessible, as such it has been refactored and split up into internal and public files, with public-facing code moved to a new `core` folder and internal code moving to the private `internal` folder.
...
```typescript
// Before
import '@getivy/node-sdk/error';
import '@getivy/node-sdk/pagination';
import '@getivy/node-sdk/resource';
import '@getivy/node-sdk/uploads';

// After
import '@getivy/node-sdk/core/error';
import '@getivy/node-sdk/core/pagination';
import '@getivy/node-sdk/core/resource';
import '@getivy/node-sdk/core/uploads';
```

If you were relying on anything that was only exported from `@getivy/node-sdk/core` and is also not accessible anywhere else, please open an issue and we'll consider adding it to the public API.

#### Resource classes

Previously under certain circumstances it was possible to import resource classes like `Banks` directly from the root of the package. This was never valid at the type level and only worked in CommonJS files.
...
As part of the `core` refactor, `@getivy/node-sdk/uploads` was moved to `@getivy/node-sdk/core/uploads`
...
```typescript
import { type Uploadable, toFile } from '@getivy/node-sdk/core/uploads';
```
...
### `@getivy/node-sdk/src` directory removed

Previously IDEs may have auto-completed imports from the `@getivy/node-sdk/src` directory, however this
directory was only included for an improved go-to-definition experience and should not have been used at runtime.

If you have any `@getivy/node-sdk/src/*` imports, you will need to replace them with `@getivy/node-sdk/*`.
